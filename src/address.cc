...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host,
  ...
  if (error) {
    ZZ_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
  ...
  while (next) {
    ...
    ZZ_LOG_DEBUG(g_logger) << "family:" << next->ai_family
...
bool Address::GetInterfaceAddresses(
  ...
  if (getifaddrs(&results) != 0) {
    ZZ_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                              " err="
  ...
  try {
    ...
    ZZ_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
    freeifaddrs(results);
    return false;
  }
  freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
  ...
  if (error) {
    ZZ_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address << ", " << port
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
  ...
  if (result <= 0) {
    ZZ_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", " << port
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
  ...
  if (result <= 0) {
    ZZ_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", " << port
                           << ") rt=" << result << " errno=" << errno
                           << " errstr=" << strerror(errno);
    return nullptr;
  }
...
void UnixAddress::setAddrLen(uint32_t v) { m_len = v; }
